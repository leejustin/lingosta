...
const PrevTranslationsList = ({
  ...
  return (
    <div className="">
      <Toaster />
      <div className="mt-4 text-lg font-semibold">Previous translations:</div>
      <div className="mx-auto grid grid-cols-1 gap-4 mt-2">
        {translationsList.map((data, index) => (
          <div
            key={index}
            className="flex justify-between shadow-md h-28 p-6 rounded-3xl bg-slate-300 hover:bg-slate-400 transition"
          >
            <div
              onClick={() => handleClick(data.rawData, data.terms)}
